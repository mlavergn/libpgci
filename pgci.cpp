...
bool PgConnection::connect(unsigned char* p_username, unsigned char* p_password, unsigned char* p_database, unsigned char* p_host, int p_port)
{
  char v_conn[512];
  sprintf(&v_conn[0],"user='%s' password='%s' dbname='%s' host=%s port=%i",p_username,p_password,p_database,p_host,p_port);
...
bool PgConnection::execute(unsigned char* p_sql)
{
  ...
  if (PQresultStatus(v_res) == PGRES_TUPLES_OK || PQresultStatus(v_res) == PGRES_COMMAND_OK)
    return true;
  else
    return false;
}
...
bool PgOperation::escapeBinds()
{
  ...
  for (int i=0; i<m_bind_cnt;i++)
  {
    ...
    switch (m_bind[i]->getType())
    {
      ...
      case PG_DATE:
      case PG_VARCHAR:
        // PQescapeString makes no provision for allocating space for the escape target pointer
        // the code states to allocate a buffer of len*2+1
        m_bind[i]->m_esc = (unsigned char*)realloc(m_bind[i]->m_esc, m_bind[i]->m_len*2+1);
        m_bind[i]->m_esc[0] = 0; // NULL terminate
        m_bind[i]->m_len =
          PQescapeString((char*)m_bind[i]->m_esc,(char*)m_bind[i]->m_ptr,m_bind[i]->m_len);
      break;
      case PG_BYTEA:
        free(m_bind[i]->m_esc);
        m_bind[i]->m_esc =
          PQescapeBytea((unsigned char*)m_bind[i]->m_ptr,m_bind[i]->m_len,(size_t*)&m_bind[i]->m_len);
      break;
      default:
        return  false;
    }
...
bool PgStatement::parse(unsigned char* p_sql)
{
  ...
  while (true)
  {
    ...
    // if this is true, we are are the end of the string so isolate the string NULL terminator
    if (v_tok == NULL)
    {
...
{
  return execute(m_sql_ph,false);
}

// -------------------------------------------------------------------------

bool PgStatement::execute(unsigned char* p_sql)
{
  return execute(p_sql,true);
}
...
bool PgStatement::execute(unsigned char* p_sql, bool p_noparse)
{
  ...
  // NOTE: it is not feasible to convert to lowercase to facilitate string parsing
  ...
  if ((v_sql[0]=='U' || v_sql[0]=='u') || (v_sql[0]=='I' || v_sql[0]=='i'))
  {
#ifdef DEBUG
    ...
    if (PQresultStatus(v_res) != PGRES_COMMAND_OK)
    {
      fprintf(stderr,"ERROR: Unable to initiate transaction (PgStatement.execute)\n");
      return false;
    }
  }
  ...
  if (!p_noparse)
  {
    ...
    if (m_bind_cnt>0)
    {
      ...
      {
        fprintf(stderr,"ERROR:  Bind variable count %i differs from bind placeholder count %i (PgStatement.execute)\n",m_bind_cnt,m_offset_cnt);
        return false;
      }
      ...
      for (int i=0; i<m_bind_cnt; i++)
      {
        ...
        if (m_bind[i]->isNull())
        {
          strncat((char*)v_sql,(char*)&m_sql_ph[m_offset[0][i]],m_offset[1][i]);
      ...
      strncat((char*)v_sql,(char*)&m_sql_ph[m_offset[0][m_offset_cnt]],m_offset[1][m_offset_cnt]);
    }
    else
...
PgCursor* PgStatement::getCursor()
{
  return m_curr;
}
...
PgCursor::PgCursor(PGresult* p_res, PgPlaceholder** p_define, int p_define_cnt)
{
  m_res = p_res;
...
PgPlaceholder::PgPlaceholder(void* p_ptr, unsigned int p_len, unsigned int p_max)
{
...
PgPlaceholder::PgPlaceholder(unsigned char* p_ptr, unsigned int p_len, unsigned int p_max)
{
...
unsigned char* PgPlaceholder::PQunescapeBytea(unsigned char *strtext, size_t *retbuflen)
{
  size_t buflen;
  ...
  if(strtext == NULL)return NULL;
    buflen = strlen((const char*)strtext); /* will shrink, also we discover if strtext */
...
{
  // PQescapeString makes no provision for escaping newlines or tabs, this give the COPY mechanism
  // a real hangover (eg: CopyReadNewline: extra fields ignored) so we need to escape those
  // ourselves, the proper thing appears to be \n -> \ + n. Do NOT use PQescapeString, it really
