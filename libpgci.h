...
public:
  ...
  bool connect(char *p_username, char *p_password, char *p_database) {
    return connect((unsigned char *)p_username, (unsigned char *)p_password,
                   (unsigned char *)p_database);
  };
  bool connect(unsigned char *p_username, unsigned char *p_password,
               unsigned char *p_database) {
    return connect(p_username, p_password, p_database,
                   (unsigned char *)"127.0.0.1", 5432);
  }
  bool connect(unsigned char *p_username, unsigned char *p_password,
  ...
  bool connect(unsigned char *p_username, unsigned char *p_password,
               unsigned char *p_database, unsigned char *p_host, int p_port);
...
public:
  ...
  bool parse(char *p_sql) { return parse((unsigned char *)p_sql); };
  bool parse(unsigned char *p_sql);
  bool define(int p_pos, PgPlaceholder *p_placeholder, int p_type);
  bool execute();
  bool execute(char *p_sql) { return execute((unsigned char *)p_sql); };
  bool execute(unsigned char *p_sql);
  PgCursor *getCursor();
...
private:
  ...
  int m_offset[2]
              [MAX_BIND_CNT]; // marks positions in SQL statement for bind vars
  int m_offset_cnt;
